...
interface ReflectionQuizPageProps {
  onBack: () => void;
  onContinue: () => void;
}

const ReflectionQuizPage: React.FC<ReflectionQuizPageProps> = ({ onBack, onContinue }) => {
  const { quizData, updateQuizData } = useQuiz();
  const [selectedOption, setSelectedOption] = useState<string | null>(quizData.reflection || null);
  const options = ['Sí', 'No', 'No estoy seguro'];

  const handleSelection = (option: string) => {
    setSelectedOption(option);
    updateQuizData({ reflection: option });
    setTimeout(() => {
        onContinue();
  ...
  return (
    <div className="bg-complementar text-apoio min-h-screen font-sen">
      <div className="max-w-sm mx-auto py-6 px-4">
        ...
        <ProgressBar currentStep={4} totalSteps={8} activeSection={1} />

        {/* Main Content */}
        <div className="text-left">
          <h1 className="text-3xl font-playfair font-bold mb-4 leading-tight">
            "A veces no me gusta mi <span className="italic">Reflejo</span> en el espejo."
          </h1>
...
export default ReflectionQuizPage;